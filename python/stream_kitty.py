...
class DsMagicKitty(Magics):
  ...
  def neods_kitty(self, line, cell):
    ...
    col_in = "\u001b[32m"
    sys.stdout.write("\n" + col_in + "╭───────── In ─────────•••\n")
    input_cmd = re.sub(r"^\s+", "", input_code)
    input_cmd = col_in + "│ " + input_cmd
    input_cmd = re.sub(r"\s+$", "", input_cmd)
    input_cmd = re.sub(r"\n", col_in + "\n│ ", input_cmd)
    sys.stdout.write(input_cmd)
...
print(vim.g.focused)
require('notify')(nprint, '{state}', {{title='{title} : result', replace=notifiers['{notifier_id}'], timeout = tmo, max_width=3000}})
